...
def update_single_field():
    try:
        ...
        index = int(data_to_update.get('index'))
        column = data_to_update.get('column')
        value = data_to_update.get('value')

        if index is None or column is None or value is None:
            return jsonify({"status": "error", "message": "Missing index, column, or value"}), 400
        ...
        if 0 <= index < len(data):
            ...
            return jsonify({"success": False, "message": f"No record found at index {index}"}), 404
    except Exception as e:
        import traceback
        error_trace = traceback.format_exc()
        print(f"Error updating single field: {error_trace}")
        print(f"Request data: {data_to_update}")
