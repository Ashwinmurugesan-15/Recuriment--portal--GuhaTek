...
const FIELD_ORDER = [
    ...
    'Initial Screening', 'Round 1 D and T', 'Round 1 Remarks', 'Round 2 D and T', 'Round 2 Remarks', 'Final Remarks',
...
// Function to open the Round 1 Remarks modal
function openRound1RemarksModal(recordIndex) {
    const record = tableData[recordIndex];
    if (!record) {
        console.error('Record not found for index:', recordIndex);
        return;
    }

    // Populate the hidden index field
    document.getElementById('round1RemarksIndex').value = recordIndex;

    // Parse existing remarks or initialize empty object
    let remarks = {};
    const rawRemarks = record['Round 1 Remarks'];
    if (typeof rawRemarks === 'string' && (rawRemarks.startsWith('{') || rawRemarks.startsWith('['))) {
        try {
            remarks = JSON.parse(rawRemarks);
        } catch (e) {
            console.error('Error parsing Round 1 Remarks JSON:', e);
            // If parsing fails, treat the entire string as a communication remark
            remarks = { 'Communication': rawRemarks };
        }
    } else if (typeof rawRemarks === 'string' && rawRemarks.trim() !== '') {
        // If it's a non-JSON string, treat it as a communication remark
        remarks = { 'Communication': rawRemarks };
    } else {
        remarks = {}; // Fallback to empty if not a string or empty
    }

    // Populate form fields
    document.getElementById('round1Communication').value = remarks.Communication || '';
    document.getElementById('round1TechnicalAssessment').value = remarks['Technical Assessment'] || '';
    document.getElementById('round1ProblemSolving').value = remarks['Problem-Solving'] || '';
    document.getElementById('round1OverallPotential').value = remarks['Overall Potential'] || '';
    document.getElementById('round1Recommendation').value = remarks.Recommendation || '';
...
// Function to save Round 1 Remarks
document.addEventListener('DOMContentLoaded', function() {
    const saveRound1RemarksBtn = document.getElementById('saveRound1RemarksBtn');
    if (saveRound1RemarksBtn) {
        saveRound1RemarksBtn.addEventListener('click', saveRound1Remarks);
    }
});

async function saveRound1Remarks() {
    const recordIndex = document.getElementById('round1RemarksIndex').value;
    const record = tableData[recordIndex];

    if (!record) {
        console.error('Record not found for saving remarks:', recordIndex);
        return;
    }

    const remarks = {
        'Communication': document.getElementById('round1Communication').value,
        'Technical Assessment': document.getElementById('round1TechnicalAssessment').value,
        'Problem-Solving': document.getElementById('round1ProblemSolving').value,
        'Overall Potential': document.getElementById('round1OverallPotential').value,
        'Recommendation': document.getElementById('round1Recommendation').value
    };

    // Convert remarks object to JSON string
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to save Round 1 Remarks');
        }

        const result = await response.json();
        if (result.success) {
            // Update the tableData and re-render the table
            record['Round 1 Remarks'] = remarksJson;
            populateTable(tableData, currentIsAdmin); // Re-render to reflect changes

            // Hide the modal
            const modalElement = document.getElementById('round1RemarksModal');
            const modal = bootstrap.Modal.getInstance(modalElement);
            if (modal) {
                modal.hide();
            }
            alert('Round 1 Remarks saved successfully!');
        } else {
            alert('Error saving Round 1 Remarks: ' + result.message);
        }
    } catch (error) {
        console.error('Error saving Round 1 Remarks:', error);
        alert('An error occurred while saving Round 1 Remarks.');
    }
}

// Function to open the Round 1 D and T modal
function openRound1DAndTModal(recordIndex) {
    const record = tableData[recordIndex];
    if (!record) {
        console.error('Record not found for index:', recordIndex);
        return;
    ...
    document.getElementById('round1DAndTIndex').value = recordIndex;

    const rawDAndT = record['Round 1 D and T'];
    let datePart = '';
    let timePart = '';

    if (rawDAndT && typeof rawDAndT === 'string' && rawDAndT.includes('&')) {
        const parts = rawDAndT.split('&').map(p => p.trim());
        datePart = parts[0] || '';
        timePart = parts[1] || '';
    }

    document.getElementById('round1Date').value = datePart;
    document.getElementById('round1Time').value = timePart;

    const modal = new bootstrap.Modal(document.getElementById('round1DAndTModal'));
    modal.show();
}

// Function to save Round 1 D and T
document.addEventListener('DOMContentLoaded', function() {
    const saveRound1DAndTBtn = document.getElementById('saveRound1DAndTBtn');
    if (saveRound1DAndTBtn) {
        saveRound1DAndTBtn.addEventListener('click', saveRound1DAndT);
    }
});

async function saveRound1DAndT() {
    const recordIndex = document.getElementById('round1DAndTIndex').value;
    const record = tableData[recordIndex];

    if (!record) {
        console.error('Record not found for saving D and T:', recordIndex);
        return;
    }

    const date = document.getElementById('round1Date').value;
    const time = document.getElementById('round1Time').value;
    const dAndTValue = `${date} & ${time}`;

    const updatedData = {
        index: recordIndex,
        column: 'Round 1 D and T',
        value: dAndTValue
    };
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to save Round 1 D and T');
        }

        const result = await response.json();
        if (result.success) {
            record['Round 1 D and T'] = dAndTValue;
            populateTable(tableData, currentIsAdmin);
            
            const modalElement = document.getElementById('round1DAndTModal');
            const modal = bootstrap.Modal.getInstance(modalElement);
            if (modal) {
                modal.hide();
            }
            alert('Round 1 Date and Time saved successfully!');
        } else {
            alert('Error saving Round 1 Date and Time: ' + result.message);
        }
    } catch (error) {
        console.error('Error saving Round 1 Date and Time:', error);
        alert('An error occurred while saving Round 1 Date and Time.');
    }
}

// Function to open the Round 2 Remarks modal
function openRound2RemarksModal(recordIndex) {
    const record = tableData[recordIndex];
    if (!record) {
        console.error('Record not found for index:', recordIndex);
        return;
    ...
    document.getElementById('round2RemarksIndex').value = recordIndex;

    let remarks = {};
    const rawRemarks = record['Round 2 Remarks'];
    if (typeof rawRemarks === 'string' && (rawRemarks.startsWith('{') || rawRemarks.startsWith('['))) {
        try {
            remarks = JSON.parse(rawRemarks);
        } catch (e) {
    ...
    document.getElementById('round2Communication').value = remarks.Communication || '';
    document.getElementById('round2TechnicalAssessment').value = remarks['Technical Assessment'] || '';
    document.getElementById('round2ProblemSolving').value = remarks['Problem-Solving'] || '';
    document.getElementById('round2OverallPotential').value = remarks['Overall Potential'] || '';
    document.getElementById('round2Recommendation').value = remarks.Recommendation || '';
    document.getElementById('round2CTC').value = remarks.CTC || ''; // New CTC field

    const modalElement = document.getElementById('round2RemarksModal');
    if (!modalElement) {
        console.error('Error: round2RemarksModal element not found!');
        return;
    }
    const modal = new bootstrap.Modal(modalElement);
    console.log('openRound2RemarksModal - Bootstrap modal object created:', modal); // Debug log
    modal.show();
}

// Function to save Round 2 Remarks
document.addEventListener('DOMContentLoaded', function() {
    const saveRound2RemarksBtn = document.getElementById('saveRound2RemarksBtn');
    if (saveRound2RemarksBtn) {
        saveRound2RemarksBtn.addEventListener('click', saveRound2Remarks);
    }
});

async function saveRound2Remarks() {
    const recordIndex = document.getElementById('round2RemarksIndex').value;
    const record = tableData[recordIndex];
    ...
    const remarks = {
        'Communication': document.getElementById('round2Communication').value,
        'Technical Assessment': document.getElementById('round2TechnicalAssessment').value,
        'Problem-Solving': document.getElementById('round2ProblemSolving').value,
        'Overall Potential': document.getElementById('round2OverallPotential').value,
        'Recommendation': document.getElementById('round2Recommendation').value,
        'CTC': document.getElementById('round2CTC').value // New CTC field
    };
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to save Round 2 Remarks');
        }

        const result = await response.json();
        if (result.success) {
            record['Round 2 Remarks'] = remarksJson;
            populateTable(tableData, currentIsAdmin);
            ...
            alert('Round 2 Remarks saved successfully!');
        } else {
            alert('Error saving Round 2 Remarks: ' + result.message);
        }
    } catch (error) {
        console.error('Error saving Round 2 Remarks:', error);
        alert('An error occurred while saving Round 2 Remarks.');
    }
}

// Function to show candidate details (vertical layout only)
function showCandidateDetails(candidate, recordIndex, isAdmin) {
    ...
    const stages = [
        ...
        { key: 'Round 2 D and T', title: 'Round 2 D and T', color: 'danger' },
                 { key: 'Round 1 D and T', title: 'Round 1 D and T', color: 'info' }
    ];
    ...
    stages.forEach(stage => {
        ...
        let text = candidate[stage.key] || '';

        if (stage.key === 'Round 1 Remarks' || stage.key === 'Round 1 D and T' || stage.key === 'Round 2 Remarks' || stage.key === 'Round 2 D and T') {
                 console.log(`Raw value for ${stage.key}:`, text); // New debug log
                 if (!text) {
                text = 'click here to fill feedback';
            } else {
                try {
                    ...
                    // JSON parsing failed, so it's not valid JSON or is just '{'
                    text = 'click here to fill feedback';
                }
            }
        }
        ...
        if (stage.key === 'Round 1 Remarks') {
            content.classList.add('clickable-remarks'); // Add a class for styling
            content.style.cursor = 'pointer'; // Indicate it's clickable
            content.addEventListener('click', () => {
                openRound1RemarksModal(recordIndex); // Use the recordIndex passed to showCandidateDetails
            });
        } else if (stage.key === 'Round 1 D and T') {
            content.classList.add('clickable-remarks'); // Add a class for styling
            content.style.cursor = 'pointer'; // Indicate it's clickable
            content.addEventListener('click', () => {
                openRound1DAndTModal(recordIndex); // Call the new function for Round 1 D and T
            });
        } else if (stage.key === 'Round 2 Remarks') {
            content.classList.add('clickable-remarks'); // Add a class for styling
            content.style.cursor = 'pointer'; // Indicate it's clickable
            content.addEventListener('click', () => {
                openRound2RemarksModal(recordIndex); // Call the new function for Round 2 Remarks
            });
        } else if (stage.key === 'Round 2 D and T') {
            content.classList.add('clickable-remarks'); // Add a class for styling
            content.style.cursor = 'pointer'; // Indicate it's clickable
            content.addEventListener('click', () => {
                openRound2DAndTModal(recordIndex); // Call the new function for Round 2 D and T
            });
        }
...
function populateTable(data, isAdmin) {
    ...
    let columnsToShow = isAdmin ? FIELD_ORDER : ['Date', 'Name', 'Email ID', 'Interested Position', 'Current Role', 'Current Organization', 'Current Location', 'Total Years of Experience', 'Resume', 'Referred By', 'Interview Status', 'Application Status', 'Initial Screening', 'Round 1 D and T', 'Round 1 Remarks', 'Round 2 D and T', 'Round 2 Remarks'];
    ...
    if (data && data.length > 0) {
        ...
        data.forEach((row, index) => {
            ...
            */
            ...
            // Add a single click listener to the row for delegation
            tr.addEventListener('click', (event) => {
                const clickedCell = event.target.closest('td');
                if (clickedCell) {
                    const columnIndex = clickedCell.dataset.column;
                    const recordIndex = getRecordIndex(row); // Ensure recordIndex is available

                    if (columnIndex === 'Round 1 Remarks') {
                      openRound1RemarksModal(recordIndex); // Use the 'index' from the outer forEach
                      event.stopPropagation(); // Prevent row click from triggering showCandidateDetails
                  } else if (columnIndex === 'Round 2 Remarks') {
                      openRound2RemarksModal(recordIndex);
                      event.stopPropagation();
                  } else if (!event.target.closest('button') && !event.target.closest('select')) {
            ...
            tableColumns.forEach((column, colIndex) => {
                ...
                td.dataset.column = column; // Add data-column attribute
                ...
                if (column === 'Candidate') {
                    ...
                    if (row[column] && row[column].toString().startsWith('http')) {
                        ...
                        td.appendChild(link);
                    } else if (column === 'Round 1 Remarks' || column === 'Round 2 Remarks') {
                        let displayValue = row[column] || '';
                        try {
                            const parsed = JSON.parse(displayValue);
                            if (typeof parsed === 'object' && Object.keys(parsed).length === 0) {
                                displayValue = 'click here to fill feedback';
                            } else if (typeof parsed === 'object') {
                                // If it's a non-empty JSON object, display a summary or a generic message
                                // For now, let's just display "Feedback Available"
                                displayValue = 'Feedback Available';
                            }
                        } catch (e) {
                            // Not a valid JSON, display as is or 'click here to fill feedback' if empty
                            if (!displayValue.trim()) {
                                displayValue = 'click here to fill feedback';
                            }
                        }
                        td.textContent = displayValue;
                        td.classList.add('clickable-remarks'); // Add a class for styling
                        td.style.cursor = 'pointer'; // Indicate it's clickable
                        td.addEventListener('click', (event) => {
                            const recordIndex = getRecordIndex(row);
                            if (column === 'Round 1 Remarks') {
                                openRound1RemarksModal(recordIndex);
                    ...
                    td.textContent = 'click here to fill feedback';
                    td.style.maxWidth = '200px';
                    td.style.overflow = 'hidden';
                    td.style.textOverflow = 'ellipsis';
                    td.style.whiteSpace = 'nowrap';
                    td.title = row['Round 1 Remarks'] || ''; // Show full text on hover
                    td.classList.add('round1-remarks-cell'); // Add class for styling and event delegation
                    td.dataset.recordIndex = index; // Store the index of the record
                    td.dataset.column = column;
                    td.dataset.column = column;
                } else if (column === 'Round 2 Remarks') {
                    ...
                    td.classList.add('round2-remarks-cell'); // Add class for styling and event delegation
                    td.dataset.recordIndex = index; // Store the index of the record
                    td.dataset.column = column;
